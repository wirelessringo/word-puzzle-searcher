...
#[structopt(name = "Word Puzzle Searcher", about = "A search program for word puzzle games")]
enum Opt {
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    match opt {
        Opt::Generate { output, file} => {
            println!("Generating a dictionary file ({:?}) from {:?}...", output, file);
        ...
        Opt::Search {
            ...
            let mut words = dict.par_iter()
                .filter(|entry| letter_count.contains(entry.count_set))
                .map(|entry| entry.word)
                .filter(|word|
                    word.len() >= min_length
                    && (if let Some(max_length) = max_length { word.len() <= max_length } else { true })
                )
                .collect::<Vec<_>>();

            words.par_sort_unstable();
            words.iter().for_each(|word| print!("{}{}", word, separator));
        },
    }
